...
public struct KrulWebRenderer: UIViewRepresentable {
    public let config: KrulLaunchConfiguration
    @ObservedObject public var model: KrulWebSurfaceModel

    public init(config: KrulLaunchConfiguration, model: KrulWebSurfaceModel) {
        self.config = config
        self.model = model
    }

    public func makeCoordinator() -> Coordinator {
        Coordinator(model: model, sessionStore: KrulSessionStore(storageKey: config.resumeStorageKey))
    }

    public func makeUIView(context: Context) -> WKWebView {
        let webView = WKWebView(frame: .zero, configuration: KrulWebViewFactory.makeConfiguration())
        ...
        let sessionStore = KrulSessionStore(storageKey: config.resumeStorageKey)
    ...
    public final class Coordinator: NSObject, WKNavigationDelegate, WKUIDelegate, UIDocumentPickerDelegate {
        private let model: KrulWebSurfaceModel
        private let sessionStore: KrulSessionStore
        private var fileSelectionHandler: (([URL]?) -> Void)?

        init(model: KrulWebSurfaceModel, sessionStore: KrulSessionStore) {
            self.model = model
        ...
        public func webView(
            ...
            guard let presenter = webView.krulTopViewController() else {
                fileSelectionHandler = nil
        ...
        private func copyToTemporaryUploadDirectory(_ sourceURL: URL) -> URL? {
            ...
            let directoryURL = FileManager.default.temporaryDirectory
                .appendingPathComponent("krul-launch-uploads", isDirectory: true)
...
private extension WKWebView {
    func krulTopViewController() -> UIViewController? {
...
@MainActor
public final class KrulWebSurfaceModel: ObservableObject {
